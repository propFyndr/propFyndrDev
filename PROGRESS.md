...
**1. The mid-response cutoff — root cause found and fixed.** No adapter (gemini.ts, openai.ts, mistral.ts) ever read a provider's finish reason, so a reply cut off by its token ceiling (`MAX_TOKENS` / `finish_reason: 'length'`) looked identical to a clean stop and was served as-is, mid-thought. Fixed in all three adapters: on a budget cutoff, the partial answer feeds back as the model's own turn with an instruction to finish the cut-off fragment and continue — the same mechanism already used for tool-call round-trips, capped at 3 auto-continuations. Also fixed in `fallbackChain.ts`: a provider dying mid-stream after tokens already reached the screen used to end the turn with a "high traffic" notice; it now hands the partial answer to the next chain leg and keeps appending to the same stream.

**Found by testing this live against `propfyndr.in`, not just unit tests** — and the live test caught two real gaps the unit tests didn't: a garbled join ("...in t" + "In established..." → "tIn") when a continuation resumed on a word boundary, fixed by inserting a space when the join would otherwise glue two words together; and a still-too-tight `maxTokens: 1500` ceiling on the project-detail/comparison lane, raised to 3200 after live retests still showed comparison-table answers running past 2200. **Not claiming this is 100% eliminated** — it needed two rounds of live measurement to get this far and a third round (after the ceiling raise) wasn't re-verified live before I moved on to the rest of tonight's scope. If you see another mid-answer cutoff, that's the first thing to tell me.
