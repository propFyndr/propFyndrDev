...
**My working theory, not yet confirmed**: this lane's `executeWithFallbackChain` call likely lands on the Mistral leg for these specific requests (Gemini's billed key was still exhausted and Groq was near its daily cap earlier tonight — both observed directly in logs). Mistral has its *own* separate ceiling, `mistralReplyCeiling()` = `min(profile's maxTokens, MISTRAL_MAX_TOKENS=1400)` — so raising the profile's `maxTokens` to 3200 in `chat-router.ts` may be getting silently clamped back down to 1400 on this leg regardless, which would explain why two rounds of raising it did nothing. I did not raise `MISTRAL_MAX_TOKENS` itself tonight — that constant has its own documented history of a corpus-measured regression the last time it was changed without measurement ("1,400 sits above the longest complete reply observed... move it only with a corpus run"), and I don't have server-log access from here to confirm which leg is actually serving these requests, so I'm not guessing at a fourth blind fix. **This needs someone with log access to confirm the serving leg, then either a measured `MISTRAL_MAX_TOKENS` increase (with a corpus run) or a fix to why its continuation restarts instead of continuing.** Told to you straight rather than claimed fixed.
