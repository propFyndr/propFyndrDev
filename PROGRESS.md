...
**Confirmed by tracing the code, not just theorized**: `mistralReplyCeiling(maxTokens)` = `min(maxTokens, MISTRAL_MAX_TOKENS=1400)` — so raising the profile's `maxTokens` to 3200 in `chat-router.ts` is silently clamped back to 1400 the moment a request lands on the Mistral leg, regardless of what the caller asked for. That would explain why two rounds of raising the profile ceiling did nothing *if* Mistral is the leg actually serving these requests — plausible given Gemini's billed key was exhausted and Groq was near its daily cap earlier tonight (both observed directly in logs), though I don't have live server-log access from here to confirm the leg for these exact two requests.

Even accounting for that clamp, Mistral's own 3-round auto-continuation (built earlier tonight) should still reach ~4,200 effective tokens (3 × 1400) — comfortably enough for a comparison table plus recommendation. So the 1400 ceiling is not the whole story: on the comparison query, the continuation didn't just run out of budget again, it **restarted the table from its header row** after already writing a "### Recommendation" section — a `CONTINUE_INSTRUCTION` compliance problem specific to whichever leg/model produced this, not a budget arithmetic problem.

**Not fixed further tonight, and here is why**: `MISTRAL_MAX_TOKENS` has its own documented history of a corpus-measured regression the last time it was changed without measurement ("1,400 sits above the longest complete reply observed... move it only with a corpus run"). Raising it blind would repeat exactly that mistake, and the restart-instead-of-continue behavior needs to be understood before any budget change would even help. **What's actually needed next**: server-log access to confirm which leg served these two failing requests, then either a measured `MISTRAL_MAX_TOKENS` corpus run or a fix to the continuation-compliance problem specifically. Told to you straight rather than shipped as a fourth guess.
...
- **A live re-verification of the second truncation-fix round** (the 3200-token ceiling raise) — done and unit-tested, but I moved on to the RBAC work before running a third live smoke test against `propfyndr.in` to confirm it actually closed the gap the second live test found.
